...
//#define GPIO1_BASE	0x3f000000
#define GPIO1_SIZE	44

#define GPIO1_REG_DATA	0x1C
#define GPIO1_REG_DIR	0
...
static int __init drvled_init(void)
{
	...
	if (!request_mem_region(GPIO_BASE, GPIO1_SIZE, DRIVER_NAME)) {
		pr_err("%s: Error requesting I/O!\n", DRIVER_NAME);
		result = -EBUSY;
		goto ret_err_request_mem_region;
	}

	drvled_data.regbase = ioremap(GPIO_BASE, GPIO1_SIZE);
...
err_ioremap:
	release_mem_region(GPIO_BASE, GPIO1_SIZE);
